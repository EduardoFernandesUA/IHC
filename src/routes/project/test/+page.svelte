<script lang="ts">
	...
	async function openCamera() {
		...
		let interval = setInterval( async ()=>{
			...
			for(let point of response.points[0].edges) {
				ctxPoints?.beginPath();
				ctxPoints?.moveTo(response.points[0].vertices[point[0]].x*cw, response.points[0].vertices[point[0]].y*ch);
				ctxPoints?.lineTo(response.points[0].vertices[point[1]].x*cw, response.points[0].vertices[point[1]].y*ch);
				ctxPoints?.stroke();
			}
			for(let point of response.points[0].vertices) {
				ctxPoints?.beginPath();
